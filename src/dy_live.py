...
def unPackWebcastCommonTextMessage(data):
    ...
    logger.info(
        "[unPackWebcastCommonTextMessage] [] [房间Id：" + liveRoomId + "] | " + log
    )
    return log
...
def WebcastProductChangeMessage(data):
    ...
    log = json.dumps(data, ensure_ascii=False)
    logger.info(
        "[WebcastProductChangeMessage] [] [房间Id：" + liveRoomId + "] | " + log
    )
...
def unPackWebcastUpdateFanTicketMessage(data):
    ...
    log = json.dumps(data, ensure_ascii=False)
    logger.info(
        "[unPackWebcastUpdateFanTicketMessage] [] [房间Id：" + liveRoomId + "] | " + log
    )
    return log
...
def unPackWebcastRoomUserSeqMessage(data):
    ...
    log = json.dumps(data, ensure_ascii=False)
    logger.info(
        "[unPackWebcastRoomUserSeqMessage] [] [房间Id：" + liveRoomId + "] | " + log
    )
    return log
...
def unPackWebcastSocialMessage(data):
    ...
    log = json.dumps(data, ensure_ascii=False)
    logger.info(
        "[unPackWebcastSocialMessage] [➕直播间关注消息] [房间Id："
        + liveRoomId
        + "] | "
        + log
    )
...
def unPackWebcastGiftMessage(data):
    ...
    try:
        ...
        GlobalVal.gift_value += int(data["gift"]["diamondCount"]) * int(
            data.get("totalCount", 1)
        )
        # 将消息发送到我们自己的服务器:websocket链接
        ws_sender(
            f"收到礼物: {gift_name}，礼物数量:{GlobalVal.gift_num}，礼物价值: {GlobalVal.gift_value}"
        )
    except Exception as e:
        logger.error(f"解析礼物数据出错: {e}")
    log = json.dumps(data, ensure_ascii=False)
    logger.info(
        f"[unPackWebcastGiftMessage] [直播间礼物消息{GlobalVal.gift_num}:{GlobalVal.gift_value}] [房间Id："
        + liveRoomId
        + "] "
        + log
    )
...
def unPackWebcastMemberMessage(data):
    ...
    log = json.dumps(data, ensure_ascii=False)
    logger.info(
        f"[unPackWebcastMemberMessage] [直播间成员加入: {member_num}] [房间Id："
        + liveRoomId
        + "] | "
        + log
    )
...
def unPackWebcastLikeMessage(data):
    ...
    log = json.dumps(data, ensure_ascii=False)
    logger.info(
        f'[unPackWebcastLikeMessage] [直播间点赞统计{data["total"]}] [房间Id：'
        + liveRoomId
        + "] | "
        + log
    )
...
def unPackMatchAgainstScoreMessage(data):
    ...
    log = json.dumps(data, ensure_ascii=False)
    logger.info(
        "[unPackMatchAgainstScoreMessage] [不知道是啥的消息] [房间Id："
        + liveRoomId
        + "] | "
        + log
    )
